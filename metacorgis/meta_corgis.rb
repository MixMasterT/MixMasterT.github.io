...
class MetaCorgiSnacks
  ...
  def method_missing(name, *args)
    raise "box_id out of range" unless @box_id.between?(1, 3)
    name = name.to_s
    box = @snack_box[@box_id]
    snack = box[name]
    tastiness = snack['tastiness']
    (tastiness > 30 ? "*" : "" ) + "#{name}: #{snack['info']}: #{tastiness}"
  end

  def self.define_snack(name)
    define_method(name) #I see no reason why to do this....
  end
end
